...
`r .kwallm_report_text_count_summary(analysis_result)` Het LLM (model: `r htmltools::htmlEscape(main_model)`) kreeg de vraag om elke tekst in te delen in `r if (assign_multiple_categories) "één of meer van" else "één van"` de volgende `r length(categories)` categorieën:
...
if (has_paragraphs) {
  cat(paste0(
    "Met de teksten waaraan categorieën zijn toegewezen, heeft een taalmodel ",
  "(", htmltools::htmlEscape(main_model), ") tenslotte een rapport geschreven.", "\n",
    "Per categorie is met quotes samengevat wat er is gezegd."
  ))
}
```
...
if (!is.null(reliability_summary)) {
  cat(paste0(
    ...
    if (assign_multiple_categories) " tekst-categoriebeslissingen." else " teksten.",
...
if (!assign_multiple_categories) {
  report_df |>
    ...
    dplyr::arrange(dplyr::desc(Aantal), Categorie) |>
    DT::datatable(
      rownames = FALSE,
      extensions = 'Buttons',
...
output <- append(output, list(
  ...
  htmltools::HTML("<p>Onderstaande paragrafen zijn de samenvattingen van de teksten per categorie.</p>"),
  htmltools::div(
  ...
  if (identical(paragraph_row$source_coverage[[1]], "sampled")) {
    warning_sampled_sources <- htmltools::div(
      ...
      htmltools::tags$small(htmltools::tags$i(
        ...
        " pasten, is deze samenvatting gebaseerd op een willekeurige steekproef van de categorieteksten."
      ))
    )
  }
