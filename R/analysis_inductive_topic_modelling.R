...
assign_topics <- function(
    ...
    if (assign_multiple_categories) {
      for (topic in topics) {
        if (!topic %in% names(df)) {
          df[[topic]] <- rep(NA, nrow(df))
        }
        df[[topic]] <- as.logical(df[[topic]])
      }
    } else {
  ...
  build_success_row <- function(i, result) {
    ...
    if (assign_multiple_categories) {
      selected_topics <- as.character(result)
      for (topic in topics) {
        row[[topic]] <- topic %in% selected_topics
      }
    } else {
    ...
    if (length(result) == 1 && is.na(result)) {
      if (identical(failure_action, "return_decision")) {
        failure_message <- failure_message %||%
          "The topic-assignment prompt did not produce a valid result after retrying."
