...
```{r echo=FALSE, results='asis', error=TRUE}
step_1_text <- paste0(
  "* Bij stap 1 worden de unieke analyse-eenheden gegroepeerd in prompt-batches"
)
...
step_1_text <- paste0(
  step_1_text,
  ". De eerste trekking behoudt de uploadvolgorde; eventuele extra trekkingen husselen de teksten opnieuw."
)
if (!is.null(topic_draws)) {
  step_1_text <- paste0(
    step_1_text,
    " In deze run kon elke tekst maximaal ",
    topic_draws,
    if (topic_draws == 1) " keer" else " keer",
    " worden getrokken."
  )
}
if (!is.null(topic_n_batches)) {
  step_1_text <- paste0(
    ...
    " Dit leverde ",
    topic_n_batches,
    " batches voor kandidaats-onderwerpen op."
  )
}
step_1_text <- paste0(
  step_1_text,
  " Het model kreeg de vraag om mogelijke onderwerpen terug te geven die niet te specifiek en niet te algemeen zijn. Dit vormt de lijst van kandidaats-onderwerpen. Voor deze stap is het model `",
...
if (topics_human_edited || auto_added_not_applicable) {
  cat("### Herkomst van de onderwerpenlijst\n")
  if (topics_human_edited) {
    cat(
      "De definitieve onderwerpenlijst is handmatig aangepast voordat de onderwerpen aan de teksten werden toegekend.\n\n"
    )
  }
  if (auto_added_not_applicable) {
    cat(
      "Een generiek onderwerp 'Onbekend/niet van toepassing' is automatisch toegevoegd, omdat in de gereduceerde onderwerpenlijst nog geen gelijkwaardig onderwerp aanwezig was.\n\n"
    )
  }
}
```
...
if (!is.null(reliability_summary)) {
  cat(paste0(
    ...
    if (assign_multiple_categories) " tekst-onderwerpbeslissingen." else " teksten.",
...
if (!assign_multiple_categories) {
  ...
  report_df |>
    ...
    dplyr::mutate(
      Percentage = round(Aantal / nrow(report_df) * 100, 2)
    ) |>
    dplyr::arrange(dplyr::desc(Aantal)) |>
    DT::datatable(
      rownames = FALSE,
...
if (!is.null(by_column_name) && !is.null(group_lookup)) {
  ...
  cat(paste0(
    "Onderstaande tabel toont per onderwerp het percentage teksten binnen elke **",
    htmltools::htmlEscape(by_column_name),
    "**-groep waaraan dat onderwerp is toegewezen."
  ))

  if (assign_multiple_categories) {
    cat(
      " Omdat meerdere onderwerpen aan een tekst kunnen worden toegewezen, kunnen percentages over onderwerpen heen binnen een groep optellen tot meer dan 100%.\n\n"
    )
  } else {
    cat("\n\n")
  }
...
if (!is.null(by_column_name) && !is.null(group_lookup)) {
  ...
  cat(paste0(
    "Onderstaande tabel toont de ruwe aantallen en percentages binnen de groep, ",
  ...
  if (identical(paragraph_row$source_coverage[[1]], "sampled")) {
    warning_sampled_sources <- htmltools::div(
      ...
      htmltools::tags$small(htmltools::tags$i(
        "Omdat niet alle teksten in het context-window van het taalmodel",
        " pasten, is deze samenvatting gebaseerd op een willekeurige steekproef van de onderwerpteksten."
      ))
    )
  }
