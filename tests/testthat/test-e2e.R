...
test_that("{shinytest2} recording: categorization with by_column grouping variable", {
  ...
  test_data <- data.frame(
    ...
    group = c("Positive", "Negative", "Positive", "Negative", "Positive"),
  ...
  # Set mode to Categorization
  app$set_inputs(`mode-mode` = "Categorization")
  ...
  # Expect result column is present
  expect_true("result" %in% colnames(results))

  # Expect results are valid categories
  expect_true(all(results$result %in% c("Positive", "Negative")))
