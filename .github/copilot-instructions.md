...
- Async: Long-running steps use `mirai::mirai()` (see `R/module_core_processing.R`); daemons are configured via `mirai::daemons()` in entry points. Progress is written via `component_progress_bar.R`/`progress_bar_server(...).$async`, and cancellation is guarded by the vendored async interrupter in `R/utils_nanonext_reactive_channel.R`.
